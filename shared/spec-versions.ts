...
export function specTargets(spec: UiSpec): SpecTarget[] {
  const result: SpecTarget[] = [{ key: key("document"), kind: "document", id: "document", title: spec.title, stage: "domain", value: { version: spec.version, title: spec.title, notes: spec.notes, domainNotes: spec.domain.notes } }];
  ...
  for (const kind of ["actors", "externalSystems", "entities", "relations", "terms"] as const) {
    for (const item of spec.domain[kind]) {
      ...
      add(kind, item.id, item.title, "domain", parent);
  ...
  for (const flow of spec.flows) {
    ...
    add("flow", flow.id, flow.title, "flows", parent);
  ...
  for (const useCase of spec.useCases) {
    ...
    add("useCase", useCase.id, useCase.title, "useCases", parent);
  ...
  for (const screen of spec.screens) {
    ...
    add("screen", screen.id, screen.title, "screens", { ...parent, initial: stateFlow.initial, notes: stateFlow.notes });
